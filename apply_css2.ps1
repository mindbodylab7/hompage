...
$replaceBody = @"
        body {
            ...
            /* ?†Ìîå ?§Ì????§Î°ú??Î∏îÎü¨ Î∞∞Í≤Ω - Ï±ÑÎèÑ?Ä Î≤îÏúÑÎ•???Í∞ïÌïòÍ≤?*/
            background-image: 
...
"@
$replaceHeader = @"
        /* ?§Îçî - Ï¶âÏãú ?úÏãú (Glassmorphism ?ÅÏö©) */
        header {
            display: flex;
            justify-content: space-between;
            align-items: center;
            padding: 15px 30px;
            background-color: rgba(255, 255, 255, 0.5); /* ?¨Î™Ö??????∂§ */
            backdrop-filter: blur(24px); /* Î∏îÎü¨ ??Í∞ïÌïòÍ≤?*/
            -webkit-backdrop-filter: blur(24px);
            border-bottom: 1px solid rgba(255, 255, 255, 0.6);
            box-shadow: 0 4px 30px rgba(0, 0, 0, 0.05); /* Í∑∏Î¶º??Î∂ÄÍ∞?*/
            position: sticky;
            top: 0;
            width: 100%;
            box-sizing: border-box;
...
$replaceHeroBtns = @"
        .new-hero__btn {
            ...
            align-items: center;
            padding: 14px 28px;
            border-radius: 999px; /* ?†Ìîå ?§Ì???Pill */
            font-size: 16px;
            font-weight: 700;
            text-decoration: none;
            cursor: pointer;
...
$replaceCoreBlock = @"
        ...
        .core-block {
            border: 1px solid rgba(255, 255, 255, 0.8);
            border-radius: 20px;
            background: rgba(255, 255, 255, 0.45); /* ?¨Î™Ö??????∂§ */
            backdrop-filter: blur(28px); /* Î∏îÎü¨ ?∏Í≤å Ï§?*/
            -webkit-backdrop-filter: blur(28px);
            box-shadow: 0 10px 40px rgba(0, 0, 0, 0.06); /* ?ÖÏ≤¥Í∞?*/
            padding: 24px;
        }
"@
...
$replaceCoreDiffItem = @"
        .core-diff-item {
            ...
            border-radius: 16px;
            padding: 16px;
            background: rgba(255, 255, 255, 0.3); /* ???¨Î™Ö?òÍ≤å */
...
#journey .story-panel.dark {
    background-color: rgba(30, 41, 59, 0.6); /* ???¨Î™Ö?òÍ≤å */
    backdrop-filter: blur(32px); /* Î∏îÎü¨ Ï¶ùÍ? */
...
.highlight-box-yellow {
    background-color: rgba(255, 249, 196, 0.4); /* ???¨Î™Ö?òÍ≤å */
...
.booking-widget {
    max-width: 600px;
    width: 100%;
    background-color: rgba(255, 255, 255, 0.45); /* ?¨Î™Ö????∂§ */
    ...
    gap: 16px;
    box-sizing: border-box;
    box-shadow: 0 10px 40px rgba(0, 0, 0, 0.08); /* Í∑∏Î¶º??Î∂ÄÍ∞?*/
}
"@
...
.consultation-list li {
    padding: 1rem 1.2rem;
    margin: 0.8rem 0;
    background-color: rgba(255, 255, 255, 0.15); /* Ï°∞Í∏à ???†Î™Ö?òÍ≤å ?ÄÎπ?*/
    backdrop-filter: blur(16px);
    -webkit-backdrop-filter: blur(16px);
...
.credentials {
  ...
  margin: 1.5rem 0;
  padding: 1.5rem;
  background-color: rgba(255, 255, 255, 0.3); /* ?¨Î™Ö????∂§ */
  backdrop-filter: blur(20px); /* Î∏îÎü¨ Ï¶ùÍ? */
  -webkit-backdrop-filter: blur(20px);
  border-radius: 16px;
  border: 1px solid rgba(255, 255, 255, 0.9);
...
.message {
  ...
  margin: 2rem 0;
  padding: 2rem;
  background-color: rgba(227, 242, 253, 0.4); /* ?¨Î™Ö??*/
  backdrop-filter: blur(24px); /* Î∏îÎü¨ Ï¶ùÍ? */
  -webkit-backdrop-filter: blur(24px);
  border: 1px solid rgba(255, 255, 255, 0.8);
  border-radius: 16px;
  color: #0f4c81;
